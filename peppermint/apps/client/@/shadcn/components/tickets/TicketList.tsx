...
export default function TicketList({
  ...
  const PRIORITY_PT: Record<string, string> = { low: "Baixa", normal: "Normal", medium: "Média", high: "Alta", urgent: "Urgente" };
  const TYPE_PT: Record<string, string> = { support: "Suporte", bug: "Bug", incident: "Incidente", feature: "Melhoria", change: "Mudança", problem: "Problema", maintenance: "Manutenção", access: "Acesso", feedback: "Feedback" };
          ...
          <ContextMenu key={ticket.id}>
                  ...
                  <div className="flex flex-row space-x-3 items-center">
                    ...
                    {uiSettings.showType && (
                      <span className={`inline-flex items-center rounded-md px-2 py-1 capitalize justify-center w-20 text-xs font-medium ring-1 ring-inset ring-gray-500/10 bg-orange-400 text-white`}>
                        {TYPE_PT[String(ticket.type).toLowerCase()] ?? ticket.type}
                      </span>
                    )}
                    {uiSettings.showPriority && (
                      <span className={`inline-flex items-center rounded-md px-2 py-1 capitalize justify-center w-20 text-xs font-medium ring-1 ring-inset ring-gray-500/10 ${badge}`}>
                        {PRIORITY_PT[String(ticket.priority).toLowerCase()] ?? ticket.priority}
                      </span>
                    )}
            ...
            <ContextMenuContent>
              <ContextMenuItem onClick={() => onStatusChange(ticket)}>
                {ticket.isComplete ? "Reabrir Ticket" : "Fechar Ticket"}
              </ContextMenuItem>
