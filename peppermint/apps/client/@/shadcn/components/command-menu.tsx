  ...
  return (
    <>
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        <CommandInput 
          placeholder="Buscar tickets por título, ID, descrição ou responsável..." 
          value={search}
        ...
        <CommandList>
          ...
          {filteredAndGroupedTickets && (
            <>
              ...
              {filteredAndGroupedTickets.open.length > 0 && (
                <CommandGroup heading="Tickets Abertos">
                  {filteredAndGroupedTickets.open.map((ticket) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-muted-foreground">
                        #{ticket.id} • {ticket.assignedTo?.name || "Não atribuído"} • {moment(ticket.createdAt).fromNow()}
                      </span>
                    </CommandItem>
                  ))}
                </CommandGroup>
              ...
              {filteredAndGroupedTickets.closed.length > 0 && (
                <CommandGroup heading="Tickets Fechados">
          ...
          {router.pathname.includes("/issue/") &&
            router.query.id &&
            ticketsData && (
              <CommandGroup heading="Ações do Ticket">
                {/* Status Toggle */}
                <CommandItem
                  onSelect={() => {
                ...
                {priorities.map((priority) => (
                  ...
                  >
                    ...
                    <span>Definir Prioridade: {priority.label}</span>
                  </CommandItem>
                ))}
